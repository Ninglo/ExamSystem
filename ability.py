...
from PyQt5.QtWidgets import QMessageBox, QMainWindow, QApplication, QPushButton
...
class select(Ui_MainWindow,QtWidgets.QMainWindow):
    ...
    def button_exercise_click(self):
        if not (self.radioButton.isChecked() or self.radioButton_2.isChecked() or self.radioButton_3.isChecked()):
            ...
            qssFileName = "./pattern/choicetype.qss"
            qssFile = QSSLoad.readQssFile(qssFileName)
            self.c = ChoiceType()
            self.c.setStyleSheet(qssFile)
            self.c.show()
